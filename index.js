...
app.post('/kakao/skill', async (req, res) => {
  ...
  try {
    ...
    if (hasRequiredParts) {
      ...
      await appendRow([now, date, name, phone, professor, course, type, reason, userId, utterance]);
      ...
      replyText = '형식이 맞지 않아요 😥\n날짜/이름/전화번호뒷자리/교수명/강좌명/결석또는지각또는출석/사유 순서로 \'/\'로 구분해서 다시 입력해주세요.';
    }
  } catch (err) {
